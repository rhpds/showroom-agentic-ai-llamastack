...
[source,bash,role="copypaste",subs=attributes+]
----
source ./convert-keys-txt-env-vars.sh
----
...
The Route was pre-created during cluster creation within the showroom namespace.  This script just calculates it based on a pattern.
You will need access to the GUI to leave feedback 
...
CHAT_TRACE_URL=https://chatbot-8002-showroom-bwpcf-1-user1.apps.cluster-bwpcf.dynamic.redhatworkshops.io/
----
...
Use the trace id to get the specific score and comment.  If you do not provide a trace id it just defaults the last trace.


[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
./get-trace-feedback.sh <trace id>
----
...
You can also download all feedback and comments from the Langfuse Console.  Make sure to run the following script to adjust the Langfuse S3 bucket URLs. Otherwise, you will see Langfuse default to: http://langfuse-s3:9000/langfuse

[.console-input]
[source,bash,role="copypaste",subs=attributes+]
----
./create-langfuse-url.sh
----


image::langfuse-feedback-comment-1.png[]

image::langfuse-feedback-comment-2.png[]

image::langfuse-feedback-comment-3.png[]
