...
The *CrashLoopBackOff* is expected at this stage of the flow as you can likely find the following error in the logs.
...
This is because the `NEXTAUTH_URL` needs to be created. The next set of steps correct this problem. 
...
We need to update the *PLACEHOLDER_URL*
----
  langfuse:
    nextauth:
      url: "PLACEHOLDER_URL"
...
Now that you have a ready to use `values-openshift.yaml` go update the deployment
...
Wait a few moments and check the pod status via `oc get pods` 
