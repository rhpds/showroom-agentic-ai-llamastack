...
First of all, in both *Terminal 1* and *Terminal 2* login to your OpenShift cluster with provided credentials `{user}` and `{password}`:
...
And answer *y* to the *Use insecure connections*
...
You should successfully login with your user and see your project agentic-{user} who has been already created for you:
...
You can also view the ConfigMap via the `oc` command. 
...
You can review the models available on the provided link:{litellm_api_base_url}[Model as a Service - MaaS^] using the following command:
...
Review the Llama Stack Server manifest below and apply by copy and paste the command in the terminal in order to install a Llama Stack Distribution for vLLM using Model-as-a-Service (MaaS):
...
In *Terminal 2*, check that the LlamaStack pod is running, it will takes a few seconds to init and start.
...
TIP: You can stop watching the pod with CTRL+C

In *Terminal 1*, check LlamaStack service, we will use it to connect to LlamaStack within the namespace:
...
If you see the pod and service then congratulations, you have a successfully deployed Llama Stack.  In the next chapter, you will be exploring Llama Stack. 