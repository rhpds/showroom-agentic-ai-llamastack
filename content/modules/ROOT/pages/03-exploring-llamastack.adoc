...
IMPORTANT: LlamaStack will deprecated Agent API in favor of Response API. Today Agent API just wraps Response API to prepare to the migration, however in the near future Agent API will not be available, suggested solution is to use the `LegacyAgent` API or to create your own class that simply wraps Response API. See this https://developers.redhat.com/articles/2025/12/09/your-ai-agents-evolved-modernize-llama-stack-agents-migrating-responses-api#the_compatibility_bridge__emulating_legacy_apis[article^] for more examples. This lab will be updated accordingly when needed.
